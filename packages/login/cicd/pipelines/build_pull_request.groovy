      ...
      stage('Tag and push PR image to ECR') {
        container('docker') {
          ...
          withCredentials([string(credentialsId: 'nexus-npm-token', variable: 'NPM_AUTH_TOKEN'),]) {
            sh "docker build -f ./Dockerfile.prod -t login:${appTag} --build-arg NPM_AUTH_TOKEN=${env.NPM_AUTH_TOKEN} ."
            sh "docker build -f ./Dockerfile.test -t login:test-${commitSha} --build-arg NPM_AUTH_TOKEN=${env.NPM_AUTH_TOKEN} ."
          }
					sh "docker build -f ./Dockerfile.nginx -t login:${nginxTag} --add-host=app:127.0.0.1 ."

          // Tag the built images
					// sh "docker tag login:${appTag} 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:${appTag}"
					// sh "docker tag login:${nginxTag} 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:${nginxTag}"
					// sh "docker tag login:test-${commitSha} 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:test-${commitSha}"

					// sh "docker push 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:${appTag}"
					// sh "docker push 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:nginx-${commitSha}"
					// sh "docker push 914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:test-${commitSha}"
        }
      }

			stage('Deploy login review app') {
        ...
        //     sh "kubectl --namespace oneaccount-dev --context eu-west-1a-nonprod run --rm --attach=true --restart=Never --env=\"BASE_URL=https://login-oneaccount-" + env.GITHUB_PR_NUMBER + ".eu-west-1a.nonprod.aurora.tescocloud.com\" login-tests-${UUID.randomUUID().toString()} --image=914904879356.dkr.ecr.eu-west-1.amazonaws.com/oneaccount/login:test-${commitSha}"
        //   }
        // }

        // sh("curl -H \"Content-Type: application/json\" -X POST -d '[\"ready for manual review\"]' https://github.dev.global.tesco.org/api/v3/repos/oneaccount/login/issues/${env.GITHUB_PR_NUMBER}/labels?access_token=${env.GITHUB_TOKEN}")
      }
    }
  }
...
def deployToEnvironment(zone, branch, jobName, commitSha, timestamp) {
  def appName = "login-" + env.GITHUB_PR_NUMBER
  def dnsName = "login-oneaccount-" + env.GITHUB_PR_NUMBER + ".eu-west-1${zone}.nonprod.aurora.tescocloud.com"
  def chart = "helm/login-nonprod-eu-west-1" + zone + "-" + env.GITHUB_PR_NUMBER

  // Prepare env-specific configuration
  sh "mkdir -p ${chart}/; " +
    " cp helm/login/Chart.yaml ${chart}/"
  ...
  sh "render " +
    ...
    " --config helm/login/values.dev.yaml" +
  ...
  sh "helm delete --purge login-${GITHUB_PR_NUMBER}" +
