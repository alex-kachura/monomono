...
try {
    ...
    node(label) {
      ...
      stage('Remove label if exists') {
        sh("curl -H \"Content-Type: application/json\" -X DELETE -d '[\"ready for manual review\"]' https://github.dev.global.tesco.org/api/v3/repos/oneaccount/login/issues/${env.GITHUB_PR_NUMBER}/labels/ready%20for manual%20review?access_token=${env.GITHUB_TOKEN} || true")
      }

			stage('Purge login deploy of closed pr') {
